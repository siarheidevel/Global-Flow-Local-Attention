...
class Dior3Dataset(BaseDataset):
    ...
    def __getitem__(self, index):         
        if index < self.pairs_df.shape[0] and self.opt.inpaint_percentage<1:
            ...
            try:
                Dense1 =np.load(P1_name+ '.dense.npz')
            except:
                Dense1={'part':np.zeros(P1_img.shape[:2],dtype=np.uint8),'uv':np.zeros((2,*P1_img.shape[:2]),dtype=np.float16)}
            ...
            try:
                Dense2 =np.load(P2_name+ '.dense.npz')
            except:
                Dense2={'part':np.zeros(P1_img.shape[:2],dtype=np.uint8),'uv':np.zeros((2,*P1_img.shape[:2]),dtype=np.float16)}
            ...
            mask = Dior3Dataset.random_ff_mask(*self.opt.refit,MAXVERTEX=8,MAX_ANGLE=60)
            try:
                Dense1 =np.load(P1_name+ '.dense.npz')
            except:
                Dense1={'part':np.zeros(P1_img.shape[:2],dtype=np.uint8),'uv':np.zeros((2,*P1_img.shape[:2]),dtype=np.float16)}
        ...
        SP1_onehot = Dior3Dataset.obtain_seg_one_hot(SP1_seg, len(SEG.labels))
        if SP2_seg is SP1_seg:
            SP2_onehot = SP1_onehot
        else:
            SP2_onehot = Dior3Dataset.obtain_seg_one_hot(SP2_seg, len(SEG.labels))
        ...
        BP1 = Dior3Dataset.obtain_bone(BP_1_array, self.opt.refit[0],self.opt.refit[1], affine_transform_1, sigma=6)  
        if  BP_2_array is BP_1_array  and  affine_transform_2 is affine_transform_1:
            BP2 = BP1
        else:
            BP2 = Dior3Dataset.obtain_bone(BP_2_array, self.opt.refit[0],self.opt.refit[1], affine_transform_2, sigma=6)

        
        dens1 = Dior3Dataset.obtain_dense_one_hot(Dense1,affine_transform_1,dstSize)
        dens2 = Dior3Dataset.obtain_dense_one_hot(Dense2,affine_transform_2,dstSize)
        
        # cv2.imwrite('augm_dens1.jpg',resized_img1*0.3+np.repeat(np.sum(dens1, axis=0)[...,np.newaxis],3,-1)*127)
        # cv2.imwrite('augm_dens2.jpg',resized_img2*0.3+np.repeat(np.sum(dens2, axis=0)[...,np.newaxis],3,-1)*127)
        ...
        return {'P1': P1, 'BP1': torch.Tensor(BP1),'D1':torch.Tensor(dens1), 'SP1': torch.Tensor(SP1).type(torch.bool), 
                'P2': P2, 'BP2': torch.Tensor(BP2),'D2':torch.Tensor(dens2),
                'M1' : torch.Tensor(mask).type(torch.bool), 'SP2': torch.Tensor(SP2).type(torch.bool),
    ...
    def _get_affine_stransform(self, img_height, img_width):
        ...
        affine_matrix = Dior3Dataset.get_affine_matrix(center=center, fit=(fit_height, fit_width), angle=angle, translate=shift,
    ...
    @staticmethod
    def obtain_dense_one_hot(dense,affine_transform,dstSize):
        p,u,v=dense['part'],dense['uv'][0].astype('float32'),dense['uv'][1].astype('float32')
        p = cv2.warpAffine(p, affine_transform[:2], dstSize,
                flags=cv2.INTER_NEAREST,
                borderMode=cv2.BORDER_CONSTANT,borderValue=0)
        u = cv2.warpAffine(u, affine_transform[:2], dstSize,
                flags=cv2.INTER_NEAREST,
                borderMode=cv2.BORDER_CONSTANT,borderValue=0)
        v = cv2.warpAffine(v, affine_transform[:2], dstSize,
                flags=cv2.INTER_NEAREST,
                borderMode=cv2.BORDER_CONSTANT,borderValue=0)
        one_hot = np.zeros((24*2,*p.shape), dtype=np.float)
        for i in range(0,24):
            one_hot[2*i]=u*(p==(i+1))
            one_hot[2*i+1]=v*(p==(i+1))
        return one_hot
