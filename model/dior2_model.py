...
import model.networks.resnet50_ft_dag as face_network
...
class Dior2(BaseModel):
    ...
    def modify_options(parser, is_train=True):
        ...
        parser.add_argument('--lambda_cx', type=float, default=0.2, help='weight for the VGG19 context loss')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['l1', 'content_gen', 'style_gen', 'cx_gen', 'mask',
                            'g_pb', 'g_pp', 'g_ps', 
                            'd_pb', 'd_pp', 'd_ps']
        # self.loss_names = ['l1', 'content_gen',  'cx_gen', 'mask',
        #                     'g_pb', 'g_pp', 'g_ps', 
        #                     'd_pb', 'd_pp', 'd_ps']
        # self.loss_names = ['l1', 'content_gen', 'style_gen', 'cx_gen', 'mask',
        #                     'g_pb', 'g_pp', 'g_ps', 'g_pb_features', 'g_pp_features', 'g_ps_features',
        ...
        #     ,
        self.visual_names = ['input_P1','input_P2','img_gen', 'input_BP1', 'input_BP2','input_SP1','input_SP2','input_M1',
            'inter_images','flow_fields','soft_mask_list'
            ]
        ...
        self.vgg.eval()
        self.face_model = face_network.resnet50_ft_dag('./saved_models/resnet50_ft_dag.pth').to(opt.device)
        ...
        if self.isTrain:
            ...
            if opt.with_D_PB:                
                ...
                self.net_D_PB=init_network(external_function2.PatchDiscriminatorWithLayers(
                    input_nc=3+18, ndf=64, img_f=256, layers=3, n_res=2, multi_dim_out=True,
            ...
            if opt.with_D_PS:
                ...
                self.net_D_PS=init_network(external_function2.PatchDiscriminatorWithLayers(
                    input_nc=3+len(SEG.labels), ndf=64, img_f=256, layers=3, n_res=2, multi_dim_out=True,
                    norm='instance', activation='LeakyReLU', use_spect=opt.use_spect_d,
                    use_coord=False), opt)
            # photo conditioned
            if opt.with_D_PP:
                ...
                self.net_D_PP=init_network(external_function2.PatchDiscriminatorWithLayers(
                    input_nc=3+3, ndf=64, img_f=256, layers=3, n_res=2, multi_dim_out=True,
                    norm='instance', activation='LeakyReLU', use_spect=opt.use_spect_d,
                    use_coord=False), opt)
        ...
        if self.isTrain:
            ...
            self.PercepLoss = external_function2.PerceptualLoss().to(opt.device)
            self.StyleLoss = external_function2.StyleLoss().to(opt.device)
            ...
            # base_function._freeze(self.net_DiorG)
            # define the optimizer torch.optim.Adam
            # https://github.com/juntang-zhuang/Adabelief-Optimizer
            # optim =partial(AdaBelief, lr=opt.lr,betas=(opt.beta1, 0.999),weight_decay=0,weight_decouple=False,eps=1e-16,rectify=True,fixed_decay=False)
            optim = partial(torch.optim.Adam,lr=opt.lr, betas=(opt.beta1, 0.999), weight_decay=0)

            self.optimizer_G =  optim(params=itertools.chain(
    ...
    def forward(self):
        ...
        # flow = torch.randn((b,2,int(h/4),int(w/4)), device=self.input_P1.device)*(1/max(h,w))
        # self.flow_fields=[F.interpolate(flow,scale_factor=0.5),flow]
        # # flow=None
    ...
    def face_features(self,x):
        b,c,h,w = x.shape
        cropped_img_gen=F.interpolate(x, scale_factor=224/min(h,w), mode ='bilinear')[:,:,:224,:224]
        normed_cropped_img_gen = (127 + 127*cropped_img_gen) - torch.tensor(
            self.face_model.meta['mean'], device=x.device)[None, :, None,None]
        face_img_gen_features=self.face_model(normed_cropped_img_gen)[1].view(b,-1)
        return face_img_gen_features
    ...
    def backward_G(self):
        ...
        vgg_features_p1 = self.vgg(self.input_P1)
        vgg_features_p2 = self.vgg(self.input_P2)

        # face_img_gen_features =self.face_features(self.img_gen)
        # face_p2_features =self.face_features(self.input_P2)
        # loss_face_=torch.nn.L1Loss()(face_img_gen_features,face_p2_features)
        # self.loss_face= loss_face_ * 0.5
        ...
        loss_content_gen =self.PercepLoss(vgg_features_img_gen, vgg_features_p2)
        loss_style_gen = self.StyleLoss(vgg_features_img_gen, vgg_features_p2)
        ...
        cx_loss = self.Cxloss(vgg_features_img_gen, vgg_features_p2,layers=['relu3_2', 'relu4_2'])
